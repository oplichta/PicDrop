...
test('users can log in', function(assert) {
  ...
  andThen(function() {
    assert.equal(find('#loginButton').length, 1, 'The page shows a login link when the session is not authenticated');
  });
...
test('users can logout', function(assert) {
  ...
  click('#logout');
  andThen(function() {
    assert.equal(find('#loginButton').length, 1, 'The page shows a login link when the users logout and session is unauthenticated');
  });
});
